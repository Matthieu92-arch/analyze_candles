...
def check_sfp_bearish(df_unique, df_check_far, df_check_close, market):
...
def check_sfp(market, df_candles, i):
    df_unique = df_candles.loc[i]
    df_check_far = df_candles.loc[i+25:i+151]
    df_check_close = df_candles.loc[i+1:i+25]

    check_sfp_bullish(df_unique, df_check_far, df_check_close, market)
    check_sfp_bearish(df_unique, df_check_far, df_check_close, market)
